...
$(function(){
	...
	// var swiper = new Swiper(".retailSwiper", {
	// 	slidesPerView: 4,
	// 	centeredSlides: true,
	// 	spaceBetween: 30,
	// 	grabCursor: true,
	...
	var TrandingSlider = new Swiper('.tranding-slider', {
	  ...
	  const prod1 = document.querySelector('.skinGuardianProduct');
	  const prod2 = document.querySelector('.skinGuardianProductBlue');
	  prod2.classList.remove('product-transition');
	  prod1.classList.remove('product-transition');

	  const observer = new IntersectionObserver(entries => {
		entries.forEach(entry => {
			// If the element is visible
			if (entry.isIntersecting) {
				prod1.classList.add('prod-transition');
				prod2.classList.add('product-transition');
				return;
			  }
			  prod1.classList.remove('prod-transition');
			  prod2.classList.remove('product-transition');
		  });
	  });
	  
	  // Tell the observer which elements to track
	  observer.observe(prod2);
	  observer.observe(prod1);
